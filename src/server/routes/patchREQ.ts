...
export const patchReq = async (_pathMap: string[], _request: any, _Database: DataManager, _config: Required<NanoWarpConfig>): Promise<Response> => {
...
const execute = async (_path: string, _request: any, _dataPath: string, _Database: DataManager, _config: Required<NanoWarpConfig>) => {
    const fullPath = `${_dataPath}/Endpoints/PATCH/${_path}`;
    ...
    try {
        ...
        if (!checkRateLimit(clientIP, `/${_path}`, rateLimitConfig)) {
            if (_config.logging) {
                console.log(setColor(`Rate limit exceeded for ${clientIP} on PATCH /${_path}`, 'red'));
            }
        ...
        if (_config.logging) {
            ...
            let debugText = setColor('Executed:', 'orange') + ' ' + setColor('PATCH', 'blue') + ' "' + setColor(fullPath, 'cyan') + '"\n';
        ...
        if (error.message === 'Request timeout') {
            if (_config.logging) {
                console.log(setColor(` ✗ Timeout: PATCH "${_path}"`, 'red'));
            }
        ...
        if (_config.logging) {
            let debugText = setColor(' Failed Request:', 'red') + ' ' + setColor('PATCH', 'blue') + ' "' + setColor(_path, 'cyan') + '"\n';
            console.log(debugText);
