...
const execute = async (_path: string, _request: any, _dataPath: string, _Database: DataManager, _config: Required<NanoWarpConfig>) => {
    ...
    try {
        ...
        if (!checkRateLimit(clientIP, `/${_path}`, rateLimitConfig)) {
            if (_config.logging) {
                console.log(setColor(`Rate limit exceeded for ${clientIP} on POST /${_path}`, 'red'));
            }
        ...
        if (_config.logging) {
            ...
            let debugText = setColor('Executed:', 'orange') + ' ' + setColor('POST', 'blue') + ' "' + setColor(fullPath, 'cyan') + '"\n';
